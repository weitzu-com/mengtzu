...
test("article routes render bilingual SEO articles from content/articles", async () => {
  ...
  const indexSource = await read("content/articles/index.ts");
  const sitemapXml = await readFile(fileURLToPath(new URL("../.next/server/app/sitemap.xml.body", import.meta.url)), "utf8");
  const llmsSource = await read("app/llms.txt/route.ts");
  const feedSource = await read("app/feed.xml/route.ts");
  const runbook = await read("docs/daily-seo-article-runbook.md");

  assert.ok(articleFiles.length >= 1);
  assert.match(llmsSource, /Articles\]/);
  assert.match(llmsSource, /专栏\]/);
  assert.match(feedSource, /articleEntries/);
  assert.match(runbook, /gsc:fetch/);
  assert.match(runbook, /images:grok/);
  assert.match(runbook, /grok-imagine-image-2\.0/);
  assert.match(sitemapXml, /<loc>https:\/\/www\.mengtzu\.com\/zh\/articles<\/loc>/);

  for (const locale of ["zh", "en"]) {
    const indexHtml = await readFile(fileURLToPath(new URL(`../.next/server/app/${locale}/articles.html`, import.meta.url)), "utf8");
    assert.match(indexHtml, /"@type":"CollectionPage"/);
    assert.match(indexHtml, /"@type":"BreadcrumbList"/);
    assert.match(indexHtml, new RegExp(`href="/${locale}/articles/[a-z0-9-]+"`));
    const homeHtml = await readFile(fileURLToPath(new URL(`../.next/server/app/${locale}.html`, import.meta.url)), "utf8");
    assert.match(homeHtml, new RegExp(`href="/${locale}/articles"`));
  }

  for (const file of articleFiles) {
    const article = JSON.parse(await readFile(path.join(articlesDir, file), "utf8"));
    assert.equal(`${article.slug}.json`, file);
    assert.ok(indexSource.includes(`./${file}`), `${file} is not registered in content/articles/index.ts`);
    assert.ok(article.keywords.length >= 3);
    assert.ok(article.en.title.length <= 60, `${file} en title too long`);
    assert.ok(article.en.description.length <= 160, `${file} en description too long`);
    assert.ok(article.zh.description.length >= 50, `${file} zh description too short`);
    assert.match(
      sitemapXml,
      new RegExp(`<loc>https://www\\.mengtzu\\.com/en/articles/${article.slug}</loc>[\\s\\S]*?<lastmod>${article.updatedAt.replace(/\./g, "\\.")}</lastmod>`),
    );

    for (const [key, image] of Object.entries(article.images)) {
      assert.ok(image.prompt.length > 40, `${file} image ${key} prompt too short`);
      assert.ok(image.alt.zh && image.alt.en, `${file} image ${key} needs bilingual alt`);
      if (image.status === "ready") {
        assert.match(image.generatedBy ?? "", /grok/i, `${file} image ${key} must be generated by Grok`);
        assert.ok(existsSync(fileURLToPath(new URL(`../public${image.src}`, import.meta.url))), `${file} image ${key} missing on disk`);
      }
    }

    for (const locale of ["zh", "en"]) {
      const html = await readFile(
        fileURLToPath(new URL(`../.next/server/app/${locale}/articles/${article.slug}.html`, import.meta.url)),
        "utf8",
      );
      assert.match(html, /"@type":"Article"/);
      assert.match(html, /"@type":"FAQPage"/);
      assert.match(html, /"@type":"BreadcrumbList"/);
      assert.match(html, new RegExp(`"dateModified":"${article.updatedAt.replace(/\./g, "\\.")}"`));
      assert.match(html, /article:modified_time/);
      assert.match(html, new RegExp(`href="/${locale}/articles"`));
      const passageLinks = html.match(new RegExp(`href="/${locale}/books/[^"]+/[^"]+"`, "g")) ?? [];
      assert.ok(passageLinks.length >= 2, `${file} ${locale} needs passage links`);
      const principleLinks = html.match(new RegExp(`href="/${locale}/principles/[^"]+"`, "g")) ?? [];
      assert.ok(principleLinks.length >= 1, `${file} ${locale} needs a principle hub link`);
      const readyImages = Object.values(article.images).filter((image) => image.status === "ready").length;
      const figures = html.match(/class="article-figure"/g)?.length ?? 0;
      assert.equal(figures, readyImages, `${file} ${locale} renders ${figures} figures for ${readyImages} ready images`);
    }
  }
});
