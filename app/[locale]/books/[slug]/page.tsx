...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: zh ? `《孟子·${name}》全文｜简体、拼音与英译` : `Mencius: ${name} — Chinese, Pinyin & English`,
    description: zh ? `《孟子·${name}》完整章句，简体中文、逐字拼音与 James Legge 公版英译对照。` : `Read ${name}, part ${book.index + 1} of the Mencius, with Chinese text, aligned pinyin, and James Legge’s English translation.`,
...
export default async function BookPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <main>
    <header className="masthead"><a className="brand" href={`/${locale}`}><span className="seal">孟</span><span>mengtzu<small>.com</small></span></a><nav><a href={`/${locale}`}>{zh ? "首页" : "Home"}</a><a href={`/${locale}#reader`}>{zh ? "十四卷" : "All parts"}</a><span className="locale-switch"><a className={zh ? "active" : ""} href={`/zh/books/${slug}`}>简体中文</a><a className={!zh ? "active" : ""} href={`/en/books/${slug}`}>English</a></span></nav></header>
    <div className="book-layout"><aside className="book-index"><div className="aside-title">{zh ? "十四卷" : "FOURTEEN PARTS"}<small>THE MENCIUS</small></div>{corpus.chapters.map((chapter, index) => <a key={chapter.id} className={index === book.index ? "active" : ""} href={`/${locale}/books/${bookSlugs[index]}`}><span>{String(index + 1).padStart(2, "0")}</span>{zh ? simplifiedBookNames[index] : chapter.name}</a>)}</aside>
      <article className="book-main"><header className="book-heading"><div className="eyebrow">{zh ? `第 ${String(book.index + 1).padStart(2,"0")} 卷 · ${book.passages.length} 章` : `PART ${String(book.index + 1).padStart(2,"0")} · ${book.passages.length} PASSAGES`}</div><h1>{displayName}</h1><p>{zh ? "原文、逐字拼音与英译对照。先读其言，再辨其所据，最后检验其原则。" : "Chinese original, character-aligned pinyin, and English translation. Read the words, expose the premise, then test the principle."}</p></header>
        <div className="book-passages">{book.passages.map((passage) => <section key={passage.ref} id={passage.ref.replace(".", "-")} className="book-passage"><div className="book-ref">{passage.ref}</div><div><RubyLine passage={passage} locale={locale}/><p className="book-english">{passage.english}</p></div></section>)}</div>
