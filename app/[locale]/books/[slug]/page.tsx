...
export default async function BookPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <main>
    <header className="masthead"><a className="brand" href={`/${locale}`}><span className="seal">孟</span><span>mengtzu<small>.com</small></span></a><nav><a href={`/${locale}`}>{zh ? "首页" : "Home"}</a><a href={zh ? "/zh/books" : "/en#reader"}>{zh ? "十四卷" : "All parts"}</a>{zh && <><a href="/zh/method">读法</a><a href="/zh/sources">版本</a></>}<span className="locale-switch"><a className={zh ? "active" : ""} href={`/zh/books/${slug}`}>简体中文</a><a className={!zh ? "active" : ""} href={`/en/books/${slug}`}>{zh ? "英文版" : "English"}</a></span></nav></header>
